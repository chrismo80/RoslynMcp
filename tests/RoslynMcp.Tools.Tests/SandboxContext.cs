...
public abstract class SandboxContext : IAsyncDisposable
{
    ...
    public string SolutionPath => Sandbox?.SolutionPath ?? throw new InvalidOperationException("The tools test sandbox has not been initialized.");

    public string TestSolutionDirectory => Sandbox?.SolutionRoot ?? throw new InvalidOperationException("The tools test sandbox has not been initialized.");

    public RoslynMcp.Tools.Inspection.LoadSolution.Result LoadedSolution { get; private set; } = default!;
    ...
    protected async Task InitializeSandboxAsync(TestSolutionSandbox sandbox, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            LoadedSolution = await GetRequiredService<RoslynMcp.Tools.Inspection.LoadSolution.Tool>()
                .Run(cancellationToken, SolutionPath)
