...
public sealed class Service(RoslynMcp.Tools.Infrastructure.Services.Workspace workspace)
{
	...
	public async Task<Result> RunAsync(Request request, CancellationToken cancellationToken)
	{
		...
		if (filters.MaxFindings is not null)
			prioritized = prioritized.Take(filters.MaxFindings.Value).ToArray();
	...
	private async Task<IReadOnlyList<Match>> CollectMatchesAsync(Document document, List<string> warnings, CancellationToken cancellationToken)
	{
	...
	private static IReadOnlyList<CodeSmellFindingEntry> Aggregate(IReadOnlyList<Match> matches)
		=> matches
			...
			.Select(static group => CreateFinding(group.ToArray()))
	...
	private static CodeSmellFindingEntry CreateFinding(IReadOnlyList<Match> matches)
	{
		...
		var occurrenceFiles = matches
			...
			.Select(static group => new CodeSmellOccurrenceFile(group.Key, group.Select(static location => new ReferencePosition(location.Line, location.Column)).ToArray()))
			.ToArray();
	...
	private static (Filters? Filters, Result? Error) Validate(Request request)
	{
		...
		var riskLevels = NormalizeSet(request.RiskLevels, NormalizeRiskLevelFilter, "riskLevels");
		if (riskLevels.Error is not null)
			return (null, riskLevels.Error);
		...
		return (new Filters(path, request.MaxFindings, riskLevels.Values, categories.Values, reviewMode), null);
	}
