...
internal static class Extensions
{
    ...
    internal static Result WithWorkspaceRelativePaths(this Result result)
        => result with
        {
            Findings = [.. result.Findings.Select(static finding => finding.WithWorkspaceRelativePaths())],
    ...
    private static CodeSmellFindingEntry WithWorkspaceRelativePaths(this CodeSmellFindingEntry finding)
        => finding with
        {
            OccurrenceFiles = [.. finding.OccurrenceFiles.Select(static file => file with { FilePath = file.FilePath.ToWorkspaceRelativePathIfPossible() })]
        };
