...
internal static class Extensions
{
	...
	internal static async Task<Candidate[]> ResolveByQualifiedNameAsync(this string qualifiedName, IReadOnlyList<Project> projects, CancellationToken cancellationToken)
	{
		...
		var shortNameMatches = caseSensitiveMatches.Length > 0
			...
			: candidates.Where(match => string.Equals(match.Symbol.Name, query.LookupName, StringComparison.OrdinalIgnoreCase)).ToArray();
	...
	extension(ISymbol symbol)
	{
		...
		private string ResolveProjectName(Project fallbackProject)
	...
	extension(Result result)
	{
		internal Result WithWorkspaceRelativePaths()
			=> result with
			{
				...
				Candidates = result.Candidates.Select(candidate => candidate.WithWorkspaceRelativePaths()).ToArray(),
...
internal sealed class QualifiedSymbolQuery
{
	...
	private static List<string> SplitSegments(string value)
	{
		...
		return segments.Select(static segment => segment.Trim()).Where(static segment => segment.Length > 0).ToList();
	}
