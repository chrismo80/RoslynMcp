...
public sealed class Service(Infrastructure.Services.Workspace workspace, SymbolLookup symbolLookup)
{
	public async Task<Result> RunAsync(Request request, CancellationToken cancellationToken)
	{
		...
		var typeSymbol = await ResolveTypeAsync(request, session.Solution, symbolLookup, cancellationToken).ConfigureAwait(false);
		if (typeSymbol.Error is not null)
			return new Result([], 0, request.IncludeInherited, typeSymbol.Error).WithWorkspaceRelativePaths();
