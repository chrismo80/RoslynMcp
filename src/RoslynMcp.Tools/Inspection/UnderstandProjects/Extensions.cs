	...
	extension(ProjectSummary project)
	{
		public ProjectSummary WithWorkspaceRelativePaths()
			=> project with
			{
				...
				OutgoingDependencyProjectPaths = project.OutgoingDependencyProjectPaths.Select(path => path.ToWorkspaceRelativePathIfPossible()).ToArray(),
				IncomingDependencyProjectPaths = project.IncomingDependencyProjectPaths.Select(path => path.ToWorkspaceRelativePathIfPossible()).ToArray()
			};
	}
	...
	extension(Result result)
	{
		public Result WithWorkspaceRelativePaths()
			=> result with
			{
				...
				Hotspots = result.Hotspots.Select(hotspot => hotspot.WithWorkspaceRelativePaths()).ToArray(),
