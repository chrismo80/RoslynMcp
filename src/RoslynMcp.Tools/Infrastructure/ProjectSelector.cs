...
internal static class ProjectSelector
{
		...
		out ProjectSelectorError? error)
	{
		...
		if (normalizedPath is null && normalizedName is null && normalizedId is null)
		{
			...
			{
				error = null;
				return solution.Projects.OrderBy(static project => project.Name, StringComparer.Ordinal).ToArray();
			}
		...
		var matches = solution.Projects
			.Where(project => normalizedPath is null || project.FilePath.MatchesByNormalizedPath(normalizedPath) || project.FilePath?.ToWorkspaceRelativePathIfPossible().MatchesByNormalizedPath(normalizedPath) == true)
