...
internal static class Extensions
{
	...
	private static DiagnosticsDeltaInfo WithWorkspaceRelativePaths(this DiagnosticsDeltaInfo diagnostics)
		=> new(diagnostics.NewErrors.Select(static item => item with { FilePath = item.FilePath.ToWorkspaceRelativePathIfPossible() }).ToArray(), diagnostics.NewWarnings.Select(static item => item with { FilePath = item.FilePath.ToWorkspaceRelativePathIfPossible() }).ToArray());
}
