...
public sealed class Service(RoslynMcp.Tools.Infrastructure.Services.Workspace workspace, SymbolLookup symbolLookup)
{
	public async Task<Result> RunAsync(Request request, CancellationToken cancellationToken)
	{
		...
		var affected = references
			...
			.Select(static group => new AffectedFileLocations(group.Key, group.Select(static item => item.Position).OrderBy(static position => position.Line).ThenBy(static position => position.Column).ToArray()))
